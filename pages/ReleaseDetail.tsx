...
const MetaItem: React.FC<{ label: string; value?: React.ReactNode }> = ({ label, value }) => (
    <div className="group/meta">
        ...
        <p className="font-bold text-white break-words">{value || <span className="text-gray-700 italic font-normal text-xs">Not Specified</span>}</p>
    </div>
);
...
const InteractionLog: React.FC<{ notes: InteractionNote[] }> = ({ notes }) => (
    <div className="space-y-4 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
        {notes.length === 0 ? (
            <p className="text-gray-500 italic text-sm py-4">No audit history recorded.</p>
        ) : notes.map((note) => (
...
const ReleaseDetail: React.FC = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (releaseData.primaryArtistIds.length > 0) {
                    setArtist(artistMap.get(releaseData.primaryArtistIds[0]) || null);
                }
    ...
    if (!release) return <div className="text-center p-20 text-red-500 font-black uppercase tracking-widest">Session not found in distribution archive.</div>;
        ...
        <div className="space-y-8 animate-fade-in w-full pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start gap-6 border-b border-gray-800 pb-8 px-4">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h1 className="text-4xl font-black text-white tracking-tighter uppercase italic">{release.title}</h1>
                        <p className="text-gray-500 font-bold uppercase tracking-widest mt-1">
                            Primary Artist: <span className="text-primary">{artist?.name || 'Unknown'}</span>
                        </p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 px-4">
                {/* Side Panel: Assets & Audit */}
                <div className="lg:col-span-3 space-y-8">
                    <Card className="overflow-hidden border-gray-800 bg-black/40 shadow-2xl">
                        <div className="relative group">
                            ...
                            <div className="absolute bottom-4 left-4 right-4">
                                ...
                                <p className="text-xs text-white font-bold truncate">{release.artworkFileName || 'Original_Cover_Art.jpg'}</p>
                            </div>
                        </div>
                    </Card>
                ...
                {/* Main Panel: Catalog Metadata */}
                <div className="lg:col-span-9 space-y-8">
                    <Card className="border-gray-800 shadow-xl bg-gray-900/30">
                        <CardHeader className="border-b border-gray-800/50">
                            <CardTitle className="text-sm uppercase tracking-widest">Session Metadata Overview</CardTitle>
                        </CardHeader>
                    ...
                    {/* Film & Soundtrack Metadata */}
                    {(release.filmName || release.filmDirector) && (
                        <Card className="border-primary/10 bg-primary/5 shadow-xl">
                    ...
                    <Card className="border-gray-800 bg-gray-900/40">
                        <CardHeader className="border-b border-gray-800/50">
                            <CardTitle className="text-sm uppercase tracking-widest">Master Tracklist ({release.tracks.length})</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-6">
                            <div className="space-y-6">
                                {release.tracks.map((track: Track) => {
                                    ...
                                    return (
                                        <div key={track.id} className="p-8 bg-black/40 rounded-3xl border border-gray-800/60 hover:border-primary/30 transition-all group/track">
                                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                                                <div className="flex items-center gap-6">
                                                    ...
                                                    <div>
                                                        <h3 className="font-black text-white text-2xl tracking-tighter uppercase italic group-hover/track:text-primary transition-colors">
                                                            {track.title}
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-8 border-y border-gray-800/40">
                                                ...
                                                <MetaItem label="Audio Filename" value={<span className="text-xs text-gray-500 truncate block">{track.audioFileName}</span>} />
                                            </div>
                    ...
                    <Card className="border-gray-800 bg-gray-900/30">
                        <CardHeader>
                            <CardTitle className="text-sm uppercase tracking-widest">Marketing Copy / Liner Notes</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="p-6 bg-black/40 rounded-2xl border border-gray-800/40 leading-relaxed text-gray-300 whitespace-pre-wrap italic font-medium">
